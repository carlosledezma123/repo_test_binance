...
# https://api.binance.com/api/v3/ticker/price
   ...
   try:
      ...
      file_entradas = open(APP_PATH+"\\ENTRADAS_RSI_SMA_15M.LOG", "a")
      file_entradas.write("################################################################################################\n")
      file_entradas.close()
      for reg in json_response:
         ...
         and not (patronLUNC.search(reg['symbol']))  and not (patronUSTC.search(reg['symbol']))
            ...
            porc_dif_sma = (SMA_7_sec * 100) / (SMA_50_sec)
           ...
           # and (SMA_50_sec > SMA_50_sec_1)
            ...
            # and ((diferencia_SMA <= 0.05) or (diferencia_SMA >= -0.03))
            and ((porc_dif_sma <= 105) or (porc_dif_sma >= 95))
           ...
           and (VELA_VERDE_ROJA_PASADA['TIPO_VELA']=='VELA VERDE')
            and densidad_ordenes >= 50
              ...
              if (info_market['status'] == 'TRADING'):
                 ...
                 # price_ganador = round (float(price) * 1.022,4)
                 price_ganador = round (float(price) * 0.511,4)
                 ...
                 file_entradas.write(timestampStr+";COMPRA;"+str(round(float(price),4))+";GANADOR="+str(price_ganador)+";RSI ACTUAL;"+str(rsi)+
                 # ";RSI ANTERIOR;"+str(rsi_1)+
                 ";"+SYMBOL+";"+
                 # VELA_VERDE_ROJA_ACTUAL['TIPO_VELA']+
                 ";"+VELA_VERDE_ROJA_ACTUAL['CONDICION']+";DENSIDAD VENTAS="+
                 str(densidad_ordenes)+";DIF_RSI="+str(diferencia_RSI)+";DIF_SMA_PORC="+str(round(porc_dif_sma,2))+
                 # ";DIF_SMA_PASADA="+str(diferencia_SMA_1)+";SMA_actual="+str(SMA_7_sec)+";SMA_1="+str(SMA_7_sec_1)+";SMA_25_sec_actual="+str(SMA_50_sec)+";SMA_25_sec_1="+str(SMA_50_sec_1)+
                 ";PORC CAMBIO="+str(round(VELA_VERDE_ROJA_ACTUAL['PORC_CAMBIO'],2))+";PORC_PRECIO_VELA="+str(round(VELA_VERDE_ROJA_ACTUAL['PORCETAJE_PRECIO_VELA'],2))+";\n")
                 file_entradas.close()
